...
if Account is not None:
    ...
    class AccountAdmin(admin.ModelAdmin):
        list_display = ("id", "name", "owner", "type", "currency", "balance_display", "threshold_notify")
        search_fields = ("name", "owner__email", "owner__username")
        list_filter = ("type", "currency")
        actions = [export_as_csv_action(description=_("Export selected accounts as CSV"))]
...
if Transaction is not None:
    ...
    class TransactionAdmin(admin.ModelAdmin):
        # 'is_transfer' now refers to a real property on Transaction model (see models.py)
        list_display = ("id", "account", "amount", "currency", "type", "category", "date", "is_transfer", "created_by")
        search_fields = ("description", "counterparty", "id")
        list_filter = ("type", "category", "date")
        actions = [
            export_as_csv_action(description=_("Export selected transactions as CSV")),
        ...
        def action_mark_duplicates(self, request, queryset):
            ...
            if hasattr(model, "mark_duplicates"):
                try:
                    ...
                    self.message_user(request, _("Marked %d duplicates (via model method).") % marked, messages.SUCCESS)
                    return
                except Exception:
                    pass
            ...
            for tx in queryset.order_by("account_id", "date", "amount"):
                key = (tx.account_id, str(tx.amount), tx.date.isoformat() if hasattr(tx.date, "isoformat") else str(tx.date))
                if key in seen:
                    ...
                    seen.add(key)
            self.message_user(request, _("Marked %d duplicates (naive).") % marked, messages.INFO)

        action_mark_duplicates.short_description = _("Mark duplicate transactions")

        def action_recalculate_balances(self, request, queryset):
            AccountModel = _get_model("Account")
            if AccountModel is None:
                self.message_user(request, _("Account model not found; cannot recalculate balances."), messages.WARNING)
                return

            affected_accounts = set(tx.account_id for tx in queryset if getattr(tx, "account_id", None) is not None)
            recalc_count = 0
            for acc_id in affected_accounts:
                ...
                if hasattr(acc, "recalculate_balance"):
                    try:
                        ...
                        continue
            self.message_user(request, _("Recalculated balances for %d accounts.") % recalc_count, messages.SUCCESS)

        action_recalculate_balances.short_description = _("Recalculate balances for affected accounts")
...
if Adjustment is not None:
    ...
    class AdjustmentAdmin(admin.ModelAdmin):
        list_display = ("id", "account", "user", "old_amount", "new_amount", "reason", "created_at", "reversed_by")
        search_fields = ("reason", "account__name", "user__email", "id")
        list_filter = ("created_at",)
        actions = ["action_reverse_adjustments", export_as_csv_action(description=_("Export selected adjustments as CSV"))]
            ...
            with transaction.atomic():
                for adj in queryset.select_for_update():
                    ...
                    reverse_kwargs = {
                        ...
                        "user": request.user,
                        "old_amount": getattr(adj, "new_amount", None),
                        "new_amount": getattr(adj, "old_amount", None),
                        "reason": f"Reversal of adjustment {adj.pk}: {getattr(adj, 'reason', '')}",
                    }
                    try:
                        if hasattr(model, "create_reverse"):
                            rev = model.create_reverse(adj, performed_by=request.user)
                        else:
                        ...
                        self.message_user(request, _("Failed to reverse adjustment %(pk)s: %(err)s") % {"pk": adj.pk, "err": exc}, messages.ERROR)
                        continue
            self.message_user(request, _("Created %d reverse adjustments.") % created, messages.SUCCESS)

        action_reverse_adjustments.short_description = _("Create reverse adjustments for selected items")
...
if Goal is not None:
    ...
    class GoalAdmin(admin.ModelAdmin):
        list_display = ("id", "user", "name", "target_amount", "current_amount", "deadline", "is_active")
...
if ImportJob is not None:
    ...
    class ImportJobAdmin(admin.ModelAdmin):
        list_display = ("id", "owner", "status", "created_at", "completed_at", "rows_total", "rows_imported")
        search_fields = ("owner__email", "file_name")
        list_filter = ("status", "created_at")
        actions = [export_as_csv_action(description=_("Export selected import jobs as CSV"))]
...
if AuditLog is not None:
    ...
    class AuditLogAdmin(admin.ModelAdmin):
        list_display = ("id", "object_type", "object_id", "action", "actor", "created_at")
